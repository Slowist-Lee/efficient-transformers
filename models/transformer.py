...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # 1. encode the captions
        embedded_output=self.embedding(captions)
        tgt=self.positional_encoding(embedded_output)
        # 2. prepare a mask
        tgt_mask = torch.tril(torch.ones(T, T,device=captions.device)) 

        features = self.visual_projection(features).unsqueeze(1)
        tgt=self.transformer(tgt=tgt,memory=features,tgt_mask=tgt_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        embedded_image=self.positional_encoding(self.patch_embed(x))
        encoded_image=self.transformer(embedded_image)
        pooled_image=torch.mean(encoded_image,dim=1).reshape(N,-1)
        logits=self.head(pooled_image)
