...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        ############################################################################
        pos = torch.arange(max_len).unsqueeze(1) # 创建 i 向量，行
...
other_data = torch.randn(batch_size, sequence_length, embed_dim)
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # Q=self.query(query).view(N,S,self.n_head,self.head_dim).transpose(1,2) # (N,self.n_head,S,self.head_dim)
        # K=self.key(key).view(N,T,self.head_dim,self.n_head).transpose(1,3) # (N,self.n_head,self.head_dim,T) 
        # V=self.value(value).view(N,T,self.n_head,self.head_dim).transpose(1,2) # (N,self.n_head,T,self.head_dim) 
        scores=(torch.matmul(Q,K.transpose(-2,-1))/math.sqrt(self.head_dim))
        if attn_mask is not None:
            scores=scores.masked_fill(attn_mask==0, -1e9)
        softmax=nn.Softmax(dim=-1)
        attention=torch.matmul(self.attn_drop(softmax(scores)),V).transpose(1, 2).contiguous().view(N, S, E)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        num_size=self.img_size // self.patch_size
        
        out = torch.reshape(x,(N,C,num_size,self.patch_size,num_size,self.patch_size)).permute(0,2,4,1,3,5)
        out = torch.reshape(out,(N,self.num_patches,self.patch_dim))
        out = self.proj(out)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class SparseMoELayer(nn.Module):
    def __init__(self, hidden_dim, intermediate_dim, num_experts, top_k,dropout=0.1):
        super().__init__()
        ...
        self.experts = nn.ModuleList([
            Expert(hidden_dim, intermediate_dim,dropout) for _ in range(num_experts)
        ])
        # 2. 实例化路由
    ...
    def forward(self, x):
        ...
        # 将输入展平为 [total_tokens, hidden_dim]
        flat_x = x.view(-1, hidden_dim)
        
        # 初始化最终的输出张量，用 zeros_like 占位
        final_output = torch.zeros_like(flat_x)
        
        # TODO: 第一步 - 路由 (Routing)

        logits=self.router(flat_x)
        # logits 中最大的 top_k 个值及其对应的专家索引
        weights_logits,selected_experts=torch.topk(logits,self.top_k)
        weights=F.softmax(weights_logits,dim=-1)
        # 第二步 - 计算与合并 (Computation & Aggregation)
        for i, expert in enumerate(self.experts):
            token_idx,expert_idx=torch.where(selected_experts==i)
            if torch.numel(expert_idx)>0:
                tokens = flat_x[token_idx]
                expert_out = expert(tokens)
                current_weights = weights[token_idx, expert_idx].unsqueeze(-1)
                final_output[token_idx]+=expert_out * current_weights
        # 将结果还原回原来的形状 [Batch, Seq_len, Hidden_dim]
...
class MoETransformerDecoderLayer(nn.Module):
    ...
    A single layer of a Transformer decoder, to be used with TransformerDecoder.
    """
    def __init__(self, input_dim, num_heads, dim_feedforward=2048, num_experts=4, top_k=2,dropout=0.1):
        """
        Construct a TransformerDecoderLayer instance.
        ...
        self.self_attn = MultiHeadAttention(input_dim, num_heads, dropout)
        self.cross_attn = MultiHeadAttention(input_dim, num_heads, dropout)
        self.ffn = SparseMoELayer(input_dim, dim_feedforward, num_experts, top_k,dropout)
